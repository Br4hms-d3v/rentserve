...
/**
 * Service implementation for managing users. Uses UserRepository to perform database operations.
 */
@Service
public class UserServiceImpl implements UserService {
  ...
  /**
   * Register a new user. It checks email, pseudo, and age. Password is encoded before saving.
   *
  ...
  public User register(UserForm form) {
    ...
    String confirmation =
        "http://localhost:8080/api/mail/confirmation?token="
            + emailTokenDTO.confirmationToken()
            + "&email="
            + user.getEmail();
  ...
  /**
   * This method activate the user after click on his email. The account come True
   *
  ...
  public User login(UserLoginForm form) {
    Optional<User> foundEmailOrPseudo =
        userRepository.findByEmailOrPseudo(form.email(), form.pseudo());
  ...
  /**
   * Get user by ID If not found send a message exception
   *
  ...
  public UserDTO userFindById(long id) {
    ...
    User currentUser =
        userRepository.findByPseudo(currentPseudo).orElseThrow(UserNotFoundException::new);
  ...
  /**
   * Get a list of all users If the list is empty, send a message
   *
  ...
  public List<UserDTO> findAllUsers() {
    ...
    return listOfUsers.stream().map(userMapper::toDto).toList();
  }
  ...
  public List<UserRoleDTO> findUsersByRole(Role role) {
    ...
    return listUserByRole.stream().map(userMapper::listRoleToDto).toList();
  }

  /**
   * Update the profile user Check if the pseudo, email exist before save
   *
  ...
  public UserPasswordDTO changePassword(long id, UserChangePasswordForm user) {
    ...
    String warnUpdatePasswordConfirmationUrl =
