...
/**
 * Service implementation for managing Category. Uses CategoryRepository to perform database
 * operations Uses CategoryMapper to map between form to entity or dto to entity
 */
@Service
public class CategoryServiceImpl implements CategoryService {
  ...
  /**
   * Get a list of all categories If the list is empty, send a message exception
   *
  ...
  /**
   * Get category by ID If not found category send a message exception
   *
  ...
  /**
   * Create a new Category It Checks if a name of category is already existing If existing send a
   * message exception
   *
  ...
  public CategoryDTO createCategory(CategoryForm form) {
    ...
    Category categoryNameExists =
        categoryRepository.findCategoryByNameCategory(category.getNameCategory());
  ...
  public CategoryDTO updateCategory(long id, CategoryForm form) {
    ...
    Category categoryNameExists =
        categoryRepository.findCategoryByNameCategory(categoryId.getNameCategory());
  ...
  /**
   * Retrieves a list of categories If the list is empty send a message exception
   *
  ...
  /**
   * Soft dele of category If data is used by another tables send a message exception If not used by
   * another, delete it
   *
   * @param id the identifier of category
   */
  ...
  public void deleteCategory(long id) {
    ...
    try {
      ...
      categoryRepository
          .flush(); // Force to execute SQL now and after that, it sends a message exception
