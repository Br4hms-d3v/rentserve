...
/**
 * This controller manages User It has a method to display user by id It has a method to display a
 * list of all users It has a method to display a list of users by their role It has a method to
 * update a user by his is It has a method to change only the password It has a method to delete an
 * account
 *
 * @author Brahim K
 */
...
public class UserController {
  ...
  /**
   * Constructor with params It's require 4 dependencies: - user service - user mapper - user and
   * user role Assembler
   *
  ...
  public UserController(
      UserService userService, UserAssembler userAssembler, UserRoleAssembler userRoleAssembler) {
  ...
  public ResponseEntity<CollectionModel<UserRoleDTO>> getUsersByRole(@PathVariable Role role) {
    ...
    List<EntityModel<UserRoleDTO>> usersList =
        usersListByRole.stream().map(userRoleAssembler::toModel).toList();

    return ResponseEntity.ok(CollectionModel.of(usersListByRole));
  }

  /**
   * This method update some data's about the user Check the id, email and authenticate before
   * update
   *
  ...
  /**
   * Change only the password about the user Check his email and his password before to change
   *
   * @param id the identifier of user
  ...
  public ResponseEntity<?> changePassword(
      @PathVariable long id, @RequestBody @Valid UserChangePasswordForm form) {
  ...
  /**
   * Delete remove the user on the app Before to delete, the user must confirm with his email Soft
   * delete => change is active to is not activate
   *
   * @param id the identifier of user
  ...
  public ResponseEntity<?> deleteAccount(
      @PathVariable long id, @RequestBody @Valid UserDeleteForm form) {
