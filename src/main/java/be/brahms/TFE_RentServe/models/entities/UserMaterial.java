...
/**
 * This class represents the link between a user and a material. It extends the BaseEntity class and
...
public class UserMaterial extends BaseEntity {
  /** A description of the material. Stored as text, allowing for longer descriptions. */
  @Column(name = "description_material", columnDefinition = "TEXT")
  private String descriptionMaterial;

  /**
   * The price per hour for using the material. Stored as a decimal with a precision of 7 and a
   * scale of 2. This value cannot be null.
   */
  @Column(name = "price_hour_material", nullable = false, precision = 7, scale = 2)
  private BigDecimal priceHourMaterial;

  /** The state of the material (e.g., GOOD, DAMAGED). Stored as a string using the State enum. */
  ...
  // Relation ManyToOne
  /** The user who owns or offers the material. This field cannot be null. */
  @ManyToOne
  @JoinColumn(name = "user_id", nullable = false)
  private User user;

  /** The material associated with the user. This field cannot be null. */
  @ManyToOne
  ...
  // Relation OneToMany
  /**
   * A set of rentals involving this user-material. Allows tracking how the material has been
   * rented.
   */
  @OneToMany(
