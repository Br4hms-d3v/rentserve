...
/**
 * This filter checks each HTTP request for a JWT token. If the token is valid, it tells Spring
 * Security who the user is.
 */
@Component
public class JwtFilter extends OncePerRequestFilter {
  ...
  /**
   * This method runs for every HTTP request. It checks if the Authorization header contains a
   * Bearer token. If the token is valid, the user is set in the SecurityContext.
   *
  ...
  protected void doFilterInternal(
    ...
    String authorizationHeader =
        request.getHeader("Authorization"); // Get the "authorization" for the request from header
    ...
    if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
      ...
      if (authorizations.length == 2) {
        ...
        if (type.equals("Bearer")
            && !token.isEmpty()) { // Check if it's a Bearer token and not empty
          if (JwtUtil.isValidToken(token)) { // Check valid token
            String pseudo = JwtUtil.getPseudo(token);

            UserDetails user =
                userDetailsService.loadUserByUsername(pseudo); // Load user details by pseudo

            UsernamePasswordAuthenticationToken uPaT =
                new UsernamePasswordAuthenticationToken(user, null, user.getAuthorities());
            SecurityContextHolder.getContext()
                .setAuthentication(uPaT); // Set authentication in security context
          }
        }
      } else {
