...
/**
 * This assembler adds useful navigation links related to user operations such as retrieving
 * updating deleting a user changing the user's password.
 */
@Component
public class UserAssembler implements RepresentationModelAssembler<UserDTO, EntityModel<UserDTO>> {
   ...
   *
   * <p>This method adds useful links to the UserDto, like a link to the user itself and to the user
   * list.
   *
   * <p>The generated model contains links to:
   *
   * <ul>
   *   <li>Retrieve the user
   *   <li>Retrieve all users
   *   <li>Update the user
   *   <li>Change the user's password
   *   <li>Delete the user account
   * </ul>
   *
  ...
  public EntityModel<UserDTO> toModel(UserDTO user) {
    return EntityModel.of(
        ...
        linkTo(methodOn(UserController.class).changePassword(user.id(), null))
  ...
  /**
   * Converts UserPasswordDTO into an entity containing a self link to the password change
   * operation.
   *
  ...
  public EntityModel<UserPasswordDTO> toModel1(UserPasswordDTO dto) {
    return EntityModel.of(
        dto, linkTo(methodOn(UserController.class).changePassword(dto.id(), null)).withSelfRel());
  }
