...
/**
 * UserRoleAssembler is a class that helps to convert UserRoleDto objects into EntityModel objects.
 * It creates models with links for user data.
 *
 * <p>This class uses a default constructor.
 */
@Component
public class UserRoleAssembler
    implements RepresentationModelAssembler<UserRoleDTO, EntityModel<UserRoleDTO>> {
   ...
   *
   * <p>This method adds useful links to the UserRoleDto, like a link to the user itself and to the
   * user list.
   *
  ...
  public EntityModel<UserRoleDTO> toModel(UserRoleDTO userRole) {
    return EntityModel.of(
        userRole,
        linkTo(methodOn(UserController.class).getUsersByRole(userRole.role()))
            .withRel("List of user by their role "));
  }
}
